...
$endtime = date("Y-m-d H:i:s",mktime(-date("H")-1, 59, 59, date("m")  , date("d")+1, date("Y")));
...
foreach($stmt as $row) {
	if(sizeOf($list)==0){
		...
		if(sizeOf($list)>4){
			...
			$html .= '<td><a href="http://c3.eacomm.com:3001/speedreport?plate='.$list[0][0]['plate_no'].'&timerange='.date("Y-m-d", strtotime($querydate)).' '.$querystarttime.' to '.date("Y-m-d", strtotime($querydate)).' '.$queryendtime.'" target="_blank">';
...
$mpdf->Output('../files/pdf/SpeedingReport'.date('m').'-'.date('d').'-'.date('Y').'.pdf',"F");
exit;
?>